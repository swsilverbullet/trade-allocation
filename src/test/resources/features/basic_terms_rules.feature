Feature: Stock position & allocation basic rules
  ...
  Scenario Outline: What is "Target Market Value" & "Target Market Quantity"
    ...
    And an investor <Investor> can maintain a <Stock> stock up to <Target Market Quantity>

    Examples:
      | Investor | Capital  | Stock  | Current Price | Target Percent | Target Market Value | Target Market Quantity |
  ...
  Scenario Outline: Calculating allowed additional quantity when the account already holds some
    Given an investor <Investor> has an account with <Capital> capital
    ...
    Then an investor <Investor> can own <Additional Quantity> additional quantity of <Stock> stock

    Examples:
      | Investor | Capital  | Stock  | Current Price | Target Percent | Owned Quantity | Additional Quantity |
