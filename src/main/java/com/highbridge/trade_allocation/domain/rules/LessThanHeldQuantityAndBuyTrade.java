...
public class LessThanHeldQuantityAndBuyTrade implements FinalPositionErrorCondition {
    @Override
    public Boolean isSatisfiedBy(Double suggestedFinalPosition, Account account, Trade trade) {
        return suggestedFinalPosition < account.currentQuantity(trade.stock()) && trade.isBuy();
    }
}
